...
let small_search_close_button = document.querySelector(
  "#small_search_close_button"
);
...
nav_bar_items.forEach((val, idx) => {
  if (
    idx != 0 &&
    idx != nav_bar_items.length - 1 &&
    idx != nav_bar_items.length - 2
  ) {
...
function open_modal(e) {
  ...
  if (prev_open_modal_class == "") {
    document
      .querySelector(`.${open_modal_class}`)
      .classList.replace("close_modal", "open_modal");
  } else {
    document
      .querySelector(`.${prev_open_modal_class}`)
      .classList.replace("open_modal", "close_modal");
    document
      .querySelector(`.${open_modal_class}`)
      .classList.replace("close_modal", "open_modal");
  }
...
let promise = fetch(
  "https://raw.githubusercontent.com/pbakondy/ios-device-list/master/devices.json"
);
...
function show_results(e) {
  ...
  if (searched_value == "") {
    ...
    let matched_items = all_items.filter((val) =>
      val.name.toLowerCase().startsWith(searched_value)
    );
    if (matched_items.length == 0) {
      ...
      let matched_items_list = matched_items.map((val, idx) =>
        idx <= 8 ? `<li>${val.name}</li>` : ""
      );
...
let footer_content_middle_small_buttons = document.querySelectorAll(
  ".footer_content_middle_small > * > button"
);
